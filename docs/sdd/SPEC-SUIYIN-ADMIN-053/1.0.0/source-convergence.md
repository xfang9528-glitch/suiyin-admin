...
| S004 | 通用usage摘要、平表、小计排序与分页 | 真实分组结构被展开为普通表并跨租户借用部门 | updated | spec 4.1；专用renderer及evidence/usage-check.json、usage-boundary-check.json | none |
| S005 | 所有租户参考深圳统计及业务零值 | 未采集和真实空态被混用 | updated | 4本租户快照含1空态，11未采集；verification.md和usage-data-check.json | none |
