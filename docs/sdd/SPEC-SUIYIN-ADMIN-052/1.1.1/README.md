# SPEC-SUIYIN-ADMIN-052@1.1.1

所有租户销售变声统计与菜单管理同步的版本化静态原型合同；状态implemented。发布授权来自2026-09-20用户“完整推送”，工程交付not-requested，不建Issue或工程指派，不修改生产仓；静态原型交付通知按既有流程。
...
- [DR-095菜单联动](decisions/DR-095-admin-data-display-menu-management-parity.md) · [DR-093资格边界](decisions/DR-093-voice-transform-display-requires-tenant-and-account-config.md)（两份均为知识决策原文的只读冻结副本）
- [当前原型](../../../../prototype/_shell.html?tenant=yestar-sz&page=salesVoiceStats) · [静态预览入口（发布后）](https://suiyin-admin.pages.dev/prototype/_shell.html?tenant=yestar-sz&page=salesVoiceStats)
