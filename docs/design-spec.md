...
- 一般使用率按业务单位展示（如「2.00 条/人」）；明确属于构成占比的指标可展示百分号，但必须在列名与口径说明中写清分子 / 分母
- 用于「派生指标」列，纯数字无需此格式
- 首例：话术统计 `stats_script_v1.0.html`

### AI 辅助统计（.ai-assist-stats）· 统计页专用（2026-08-05）
- 入口位于「数据展示 → 话术统计」之后；菜单与 Tab 文案统一为「AI辅助统计」。
- 复用话术统计的快捷日期、自定义日期、账号 / 部门维度、部门筛选、账号关键词、横向表格与 10 / 20 / 50 分页；首次默认本月、按销售账号、20 条/页。
- 数值列统一居中：AI 直发、AI 参考、手动录入；最后一列用 `.ratio-cell` 展示「AI 辅助合计 / 占比」。
- AI 辅助合计 = AI 直发 + AI 参考；占比 = 合计 ÷ 三类消息总和，作为**构成占比**显示百分号；分母为 0 时只显示「—」，不画 0% 进度条。
- 只统计当前租户、当前日期范围内、功能上线后由销售发送的单聊文本消息；不展示消息正文、AI 建议原文、客户身份、未标记数或标签覆盖率。
- 所有租户均显示同一完整页面。没有本租户上线后数据时保留筛选栏、表头和分页结构，结果区显示普通空态；不得出现「未接入」，也不得借用成都艺星数据。
- 加载骨架、普通空态、统计失败 + 重试、无权限、非法日期与横向溢出必须分开表达；失败 / 无权限不能复用空态文案。
- 口径气泡必须说明「数据从本功能上线后开始累计，历史消息不追溯」和「本页仅统计使用量，不代表 AI 已带来等级或到店提升」。
- 行为真源：`SPEC-SUIYIN-ADMIN-009@1.0.0`；详细 PRD：`prd/ai-assisted-message-stats.md`。
...
### 顶栏企业切换器（用户下拉内）· 2026-06-18 / 2026-07-17
...
- 企业列表与 PC 端 `suiyin-pc-chat/prototype/prototype_v1.0.html` 的 `ENV_ORDER` 保持同顺序、同文案；2026-07-17 当前为 16 项：佰智德三 / 诚如风险管理事务所 / 达根斯 / 厚全鸡蛋 / 锦帛服饰 / 萌爪医生 / 南苑茶庐 / 瑞熙小刘 / 瑞熙小温 / 瑞熙小周 / 天馨健身 / uid / 彦库实业 / 艺星 / 艺星深圳 / 柚子烘焙

### 账号状态页真实管理页对齐 · 2026-07-17
- 外壳：艺星管理页场景使用左侧白底品牌区 + 顶部青绿色横条；品牌显示「艺星医美-深圳」，右上角保留房昕头像下拉。
- 默认入口：`_shell.html` 默认打开微信管理 → 账号状态，tab 文案统一为「账号状态」。
- 筛选区：浅灰背景横条，字段为人设名称 / 群组 / 状态，按钮为查询 / 重置 / 添加账号；高度 74px，页面内左右留 18px。
- 表格：宽表格 `min-width:1680px`，浅灰表头、细分割线、固定列宽；列顺序为头像 / 微信昵称 / 人设名称 / 群组归属 / 渠道归属 / 类型 / 登录设备 / 代理名称 / 自动通友开关 / 状态 / 可接待人 / 操作。
- 状态标签：自动通友与登录状态使用浅红/浅绿胶囊；类型使用个微蓝、企微橙。
- 可接待人列：单元格最多展示 3 个姓名 chip，超出时展示蓝色文字按钮「查看更多（N）」；N 是剩余未展示数量，不是总数。
- 可接待人弹窗：只读列表弹窗，不带 footer；标题为「可接待人列表 · {人设名称}」，行内展示序号、姓名、群组/渠道上下文；关闭方式为右上 × / 点击遮罩 / Esc。
