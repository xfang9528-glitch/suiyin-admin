...
window.AdminSalesUsage=(()=>{
 ...
 function render(){
  ...
  function summary(state){const header=node('div','usage-heading'),title=node('h2','','销售碎银发送消息统计'),info=node('span','usage-info');info.setAttribute('role','img');info.setAttribute('aria-label','统计说明');info.title='同一销售按部门展示发送消息数和聊天好友数，多部门销售显示合计。';info.innerHTML='<svg viewBox="0 0 16 16" aria-hidden="true"><circle cx="8" cy="8" r="7" fill="currentColor"/><path d="M8.1 6.7 7.2 11h1.3m-.1-6.5h.01" fill="none" stroke="#fff" stroke-width="1.35" stroke-linecap="round" stroke-linejoin="round"/></svg>';title.append(info);header.append(title);if(state==='success'||state==='empty'){const t=totals(groups);header.append(node('span','usage-summary','使用人数：'+t.users.toLocaleString('en-US')+' 人 · 合计：消息 '+t.messages.toLocaleString('en-US')+' 条 · 好友 '+t.friends.toLocaleString('en-US')+' 个'));}return header;}
  ...
  function draw(){
   ...
   const appendData=(row,label,messages,friends,department=false)=>{const cell=node('td','usage-department');if(department)cell.append(node('span','usage-department-tag',label));else cell.textContent=label;row.append(cell,node('td','usage-numeric',messages.toLocaleString('en-US')),node('td','usage-numeric',friends.toLocaleString('en-US')),node('td','usage-spacer',''));};
   ordered().forEach((g,index)=>{const body=node('tbody');body.dataset.salesId=g.id;const count=g.departments.length+(g.departments.length>1?1:0);g.departments.forEach((d,i)=>{const row=node('tr');row.dataset.departmentId=d.id;if(i===0){const rank=node('td','usage-group-cell',String(index+1)),sales=node('td','usage-group-cell',g.sales);rank.rowSpan=sales.rowSpan=count;row.append(rank,sales);}appendData(row,d.name,d.messages,d.friends,true);body.append(row);});if(g.departments.length>1){const subtotal=node('tr','usage-subtotal'),t=totals([g]);appendData(subtotal,'合计',t.messages,t.friends);body.append(subtotal);}table.append(body);});
