...
更新：2026-09-20。静态原型共 15 租户、752 入口、84 路由；合同为 051@1.1.0、052@1.2.0、053@1.0.2、054@1.0.0、055@1.1.0、056@1.0.0，见 [合同入口](../README.md)。以下流程均不写真实后台。
...
原始快照中的加载中不能判成确认空态。新窗口被浏览器阻止时提供目标链接。参考内容不授权跨租户借入实体选项；销售使用统计和 AI 专用统计没有参考租户数据回退。四页默认视图依据 [本轮发布勘误](../docs/sdd/pixel-correction-20260920.md)。
...
本轮本地 Chrome 已盘点 752 个入口、665 张表格，实际滚动 333 个场景无冻结失败或脚本错误，另有筛选 8/8、冻结 22 项断言和独立 9/9 检查。上一轮保留四页独立本地 Chrome 截图、与用户实站截图同尺寸测量及交互检查；实站 Chrome 连接及访问授权尚未完成，在线复核仍有缺口。此发布保存当前迭代，不能用结构或发布回执宣称 752 入口逐像素一致。
