...
2. 同名菜单按稳定 route/identity 区分；可见导航与完整库存分开。DR-095：数据展示新增项同次补齐适用租户菜单管理和已有平台定义，普通 menu 验证本租户联动；060@1.1.0 平台 allMenu 为全部租户侧栏的统一顺序、父级和显示规则，拖动一级携带二级，二级可跨组，刷新保留本地配置。原型 15 租户是验证清单，不是生产白名单。
